...
docker run -e GUNICORN_WORKERS=4 -p 8080:80 my-app

# Run with volume mount (development)
docker run -v $(pwd):/app -p 8080:80 my-app
...
1. **Clone/Fork:** Get the project
2. **Read Examples:** Check the [`example/`](example/) directory
3. **Create Your App:** Build your Python application
4. **Build Docker Image:** `docker build -t my-app .`
5. **Run & Test:** `docker run -p 8080:80 my-app`
6. **Deploy:** Push to Docker Hub or your registry
...
**Q: What if my app takes longer than 120 seconds?**  
A: Increase `GUNICORN_TIMEOUT` environment variable.

**Q: Can I use multiple workers?**  
A: Yes! Set `GUNICORN_WORKERS` environment variable (e.g., 4, 8, 16).

**Q: How do I add custom Nginx headers?**  
A: Create a custom nginx config file and mount it. See examples.

**Q: Does it support WebSockets?**  
A: Yes! Uvicorn supports WebSockets out of the box.

**Q: Can I use it with Kubernetes?**  
A: Absolutely! See the Advanced Usage section.

**Q: What about HTTPS/SSL?**  
A: Use a reverse proxy like Traefik or add Let's Encrypt via Certbot.
